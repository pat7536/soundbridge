...
export function speak(text, options = {}) {
  return new Promise((resolve) => {
    ...
    // Chrome suspends speechSynthesis when idle (e.g. after custom audio files
    // played without any TTS). resume() prevents the subsequent speak() call
    // from silently failing or waiting for an internal browser timeout.
...
/**
 * Primes the speech synthesis engine for iOS.
 *
 * iOS requires speechSynthesis.speak() to be called synchronously within
 * a user gesture. If the first speak() happens after an await (e.g. after
 * blending custom audio files), iOS silently drops the utterance.
 *
 * Call this at the very start of any async handler that will eventually
 * call speak() — before any await — to satisfy iOS's gesture requirement.
 * The primer utterance is silent and completes almost instantly.
 */
